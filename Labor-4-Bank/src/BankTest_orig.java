...
//	}
	...
	public void testBankAddAccountFirstAccount() {
		...
		try {
			...
			acs=(IAccount[])accounts.get(testee); //(Map<Long, IAccount>) accounts.get(testee);
		}catch(NoSuchFieldException exc) {
			exc.printStackTrace();
		}catch(IllegalAccessException exc) {
			exc.printStackTrace();
		}
		assertTrue(acs[0] == newAccount);	
	}
...
//		noOfAccounts = acs.size();
	...
	public void testTransferOverseedLimit() {
		...
		newAccount.setAccountNo(12345);
		testee.addAccount(newAccount);
		IAccount newAccount2 = new CheckingAccount(customer, new Date(30, 12, 2015), -500f);
		newAccount2.setAccountNo(112233);
		testee.addAccount(newAccount2);
